...
public class TaskService {
	...
	public Page<TaskResponse> list(Integer page, Integer size, Long projectId) {
		...
		Specification<Task> spec = (root, q, cb) -> projectId == null ? cb.conjunction()
	...
	public TaskResponse update(Long id, TaskRequest req) {
		Task t = taskRepo.findById(id).orElseThrow(() -> new NoSuchElementException("Task not found: " + id));

		Project p = projectRepo.findById(req.projectId())
				.orElseThrow(() -> new NoSuchElementException("Project not found: " + req.projectId()));
